...
if st.button("분석 실행"):
    ...
    for name, key in zip(["1. TRIX", "2. 후행스팬", "3. 선행스팬", "4. 꿀단타", "5. MACD/OBV"], ["S1","S2","S3","S4","S5"]):
        st.subheader(f"💡 {name}")
        ...
        st.warning(f"**상세 대응:** {STRATEGY_DETAILS[f'{name}']['대응']} (근거: {STRATEGY_DETAILS[f'{name}']['근거']})")
